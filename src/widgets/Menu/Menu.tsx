...
const StyledNav = styled.nav<{ isPushed: boolean; showMenu: boolean }>`
  ...
  left: ${({ isPushed }) => (isPushed ? `${SIDEBAR_WIDTH_FULL}px` : 0)};
  transition: top 0.2s;
  display: flex;
  ...
  padding-left: 8px;
  padding-right: 16px;
  width: ${({ isPushed }) => (isPushed ? `calc(100% - ${SIDEBAR_WIDTH_FULL}px)` : "100%")};
    ...
    <Wrapper>
      ...
      <BodyWrapper>
        <Panel
          ...
          priceLink={priceLink}
        />
        <Inner className="innerDiv" isPushed={isPushed} showMenu={showMenu}>
          {children}
        </Inner>
